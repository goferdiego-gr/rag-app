...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'PUT') {
      ...
      // Descontar stock si enviado o entregado
        ...
        if (p?.pedido_items) {
          for (const item of p.pedido_items) {
            const { data: s } = await supabase.from('stock')
              .select('cantidad')
              .eq('cedis_id', p.cedis_id)
              .eq('producto_id', item.producto_id).single();
            if (s) {
      ...
      // Si pagado: acumular ventas al vendedor y calcular comision
      if (status === 'pagado') {
        const { data: p } = await supabase.from('pedidos')
          .select('vendedor_id, usuario_id, cliente_id, total').eq('id', id).single();
        
        const vendId = p?.vendedor_id || p?.usuario_id;
        if (vendId) {
          const { data: usr } = await supabase.from('usuarios')
            .select('ventas_pagadas, compras_mes, comision_pct, rol, nivel')
            .eq('id', vendId).single();
          ...
          if (usr?.rol === 'vendedor') {
            ...
            await supabase.from('usuarios')
              .update({ ventas_pagadas: nuevasVentas }).eq('id', vendId);
            if (p.cliente_id) {
            ...
            // Aplicador o distribuidor: acumular compras
